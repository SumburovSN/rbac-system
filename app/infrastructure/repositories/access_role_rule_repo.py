...
class AccessRoleRuleRepositoryImpl(AccessRoleRuleRepository):
    ...
    def get_all(self) -> list[DomainRule]:
        ...
        return [self._to_domain_name(rule) for rule in db_rules]
    ...
    def get_by_role_and_element(self, role_id: int, element_id: int) -> DomainRule | None:
        ...
        return self._to_domain_name(db_rule) if db_rule else None
    ...
    # def has_permission(self, user_id: int, element_code: str, permission_type: str):
    #     # Проверка "all"
    #
    #     db_rules = (
    #         self.db.query(DbRule, DbRole.name, DbBusinessElement.code, DbBusinessElement.name).
    #         join(DbRole, DbRole.id == DbRule.role_id).
    #         join(DbBusinessElement, DbBusinessElement.id == DbRule.element_id).
    #         filter(DbRule.role_id == role_id, DbRule.element_id == element_id).first()
    #     )
    ...
    # def permission_exists(self, role_id: int, element_id: int, permission_type: str):
    #     db_rules = (
    #         self.db.query(DbRule, DbRole.name, DbBusinessElement.code, DbBusinessElement.name).join(DbRole,                                                                                                    DbRole.id == DbRule.role_id)
    #         .join(DbBusinessElement, DbBusinessElement.id == DbRule.element_id).
    #         filter(DbRule.role_id == role_id, DbRule.element_id == element_id).first()
    #     )

    @staticmethod
    def _to_domain_name(db_rule: DbRule) -> DomainRule:
